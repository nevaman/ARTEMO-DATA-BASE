...
Deno.serve(async (req: Request) => {
  ...
  const webhookSecrets = [
    ...
    Deno.env.get('MAKE_WEBHOOK_SECRET_SECONDARY'),
  ]
    .filter((value): value is string => typeof value === 'string')
    .map((value) => value.trim())
    .filter((value) => value.length > 0);

  if (!supabaseUrl || !supabaseServiceKey || webhookSecrets.length === 0) {
  ...
  const tokenAccepted = webhookSecrets.some((secret) => safeEquals(incomingToken, secret));
